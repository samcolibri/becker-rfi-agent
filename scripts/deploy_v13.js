...
async function main() {
  ...
  const flowXml = fs.readFileSync('/tmp/becker_rfi_v13.xml', 'utf8');
  const pkgXml = `<?xml version="1.0" encoding="UTF-8"?>
<Package xmlns="http://soap.sforce.com/2006/04/metadata">
    <types>
        <members>Becker_RFI_Lead_Routing</members>
        <name>Flow</name>
    </types>
    <version>59.0</version>
</Package>`;
  fs.writeFileSync('/tmp/flow_v13.xml', flowXml);
  fs.writeFileSync('/tmp/pkg_v13.xml', pkgXml);

  const pyScript = `
import zipfile, io
buf = io.BytesIO()
with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as z:
    z.write('/tmp/pkg_v13.xml', 'package.xml')
    z.write('/tmp/flow_v13.xml', 'flows/Becker_RFI_Lead_Routing.flow')
with open('/tmp/deploy_v13.zip','wb') as f:
    f.write(buf.getvalue())
print('ZIP created, size:', len(buf.getvalue()))
`;
  fs.writeFileSync('/tmp/make_zip_v13.py', pyScript);
  const result = execSync('python3 /tmp/make_zip_v13.py').toString();
  console.log(result.trim());

  const zipBuffer = fs.readFileSync('/tmp/deploy_v13.zip');
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const statusRes = await fetch(`${instanceUrl}/services/data/v59.0/metadata/deployRequest/${deployId}?includeDetails=true`, {
      headers: { Authorization: 'Bearer ' + token }
    });
    const status = await statusRes.json();
    const ds = status.deployResult;
    console.log('Status:', ds.status, '|', ds.numberComponentsDeployed, '/', ds.numberComponentsTotal);
    if (ds.done) {
      if (ds.success) {
        console.log('✅ v13 deployed successfully!');
      } else {
        console.error('❌ Deploy failed:');
        (ds.details?.componentFailures || []).forEach(f => console.error(' -', f.fullName, ':', f.problem));
      }
      break;
    }
  }
